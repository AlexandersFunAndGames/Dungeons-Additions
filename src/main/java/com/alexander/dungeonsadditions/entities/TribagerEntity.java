...
public class TribagerEntity extends AbstractTribalIllagerEntity implements IAnimatable {

	   AnimationFactory factory = new AnimationFactory(this);
	   
	   public TribagerEntity(EntityType<? extends TribagerEntity> p_i50189_1_, World p_i50189_2_) {
	   ...
	   protected void registerGoals() {
		      ...
		      this.goalSelector.addGoal(2, new TribagerEntity.AttackGoal(this));
	   ...
	   public static AttributeModifierMap.MutableAttribute createAttributes() {
		      return MonsterEntity.createMonsterAttributes().add(Attributes.MOVEMENT_SPEED, (double)0.3F).add(Attributes.FOLLOW_RANGE, 30.0D).add(Attributes.MAX_HEALTH, 24.0D).add(Attributes.ATTACK_DAMAGE, 5.0D);
		   }
	   ...
	   protected SoundEvent getAmbientSound() {
		...
		private <P extends IAnimatable> PlayState predicate(AnimationEvent<P> event) {
				if (this.getAttackTicks() > 0) {
					event.getController().setAnimation(new AnimationBuilder().addAnimation("tribal_illager_attack", true));
				} else if (!(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F)) {
						event.getController().setAnimation(new AnimationBuilder().addAnimation("tribal_illager_walk", true));
				} else {
					    event.getController().setAnimation(new AnimationBuilder().addAnimation("tribal_illager_idle", true));
				}
	   ...
	   class AttackGoal extends MeleeAttackGoal {
		      public AttackGoal(TribagerEntity p_i50577_2_) {
		      ...
		      protected void checkAndPerformAttack(LivingEntity p_190102_1_, double p_190102_2_) {
			         ...
			         if (p_190102_2_ <= d0 && this.isTimeToAttack()) {
			            ...
			            if (this.getTicksUntilNextAttack() <= 30) {
			            	if (TribagerEntity.this.getAttackTicks() <= 0) {
			            		TribagerEntity.this.setAttackTicks(40);
