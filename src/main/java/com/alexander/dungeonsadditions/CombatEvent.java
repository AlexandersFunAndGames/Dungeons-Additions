...
public class CombatEvent {
    ...
    public static void AreaAttack(LivingEntity v, float range, float X, float Y, float Z, float arc, float damage, double DeltaY, double knockback_reduction) {
        for (LivingEntity entityHit : v.level.getEntitiesOfClass(LivingEntity.class, v.getBoundingBox().inflate(X, Y, Z))) {
            ...
            if (entityHitDistance <= range && (entityRelativeAngle <= arc / 2 && entityRelativeAngle >= -arc / 2) || (entityRelativeAngle >= 360 - arc / 2 || entityRelativeAngle <= -360 + arc / 2)) {
                if (!v.isAlliedTo(entityHit) && !(entityHit == v)) {
                    ...
                    forceKnockback(entityHit, attackKnockback * 0.8F, ratioX, ratioZ, knockback_reduction);
